 ...
 * Auteur: Manus
 * Fonctionnalités: Redirection par option, Navigation entre sections, 100% Hors-ligne, Export CSV Fix, Insertion dynamique
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let responses = []; 
    let activeElementIndex = -1; // Index de l'élément actuellement sélectionné
    ...
    function renderElements() {
        ...
        formData.elements.forEach((el, index) => {
            ...
            node.addEventListener('click', (e) => {
                // Ne pas changer l'index si on clique sur un bouton de suppression
    ...
    function createQuestionNode(q, index, availableSections) {
        ...
        // FIX: Utilisation de querySelectorAll pour trouver le bouton de suppression correct
        const deleteBtn = node.querySelector('.footer-right .delete-btn');
        deleteBtn.onclick = (e) => {
            e.stopPropagation();
            formData.elements.splice(index, 1);
            if (activeElementIndex >= formData.elements.length) {
